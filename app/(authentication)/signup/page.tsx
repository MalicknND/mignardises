...
const getReadableAuthError = (message?: string) => {
  if (!message || !message.trim()) {
    return "Erreur d'inscription. Réessayez dans quelques secondes.";
  }

  if (message.includes("P2021") || message.toLowerCase().includes("table")) {
    return "La base de données n'est pas initialisée. Lancez la migration/synchronisation Prisma puis réessayez.";
  }
...
export default function SignUpPage() {
  ...
  const onEmailSignUp = async (formData: SignUpFormData) => {
    try {
      setIsLoading(true);
      setError("");
      ...
      await signUp.email(
        ...
        },
        {
          ...
          onError: (ctx) => {
            const message = getReadableAuthError(ctx.error?.message);
            setError(message);
      ...
      setError(
        getReadableAuthError(err instanceof Error ? err.message : undefined),
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const onGoogleSignUp = async () => {
    try {
      ...
      if (result?.error) {
        setError(getReadableAuthError(result.error?.message));
      }
    } catch (err) {
      setError(
        getReadableAuthError(err instanceof Error ? err.message : undefined),
